...
class AHNPairCylinder_DC(AHNCylinder):
    def __init__(self, nb_cluster_kmeans=2, dcva_gt=False, *args, **kwargs):
        self.n_feats = 10
        self.nameInPly="params"
        self.pseudo_labels = None
        self.pseudo_labels_past = None
        self.pseudo_label_vote = None
        self.stage_dc = "train"
        self.dcva_gt = dcva_gt
    ...
    def _get_paths(self):
        ...
        for dir in globPath:
            if dir.is_dir():
                ...
                for f in curDir:
                    if "AHN3" in f.name and "features" in f.name and "ply" in f.name:
                        self.filesPC0.append(f.path)
                    elif "AHN4" in f.name and "features" in f.name and "ply" in f.name:
    ...
    def _preproc_clouds_loader(self, area):
        ...
        return data_pc0.pos, data_pc1.pos, data_pc1.y, data_pc0.x, data_pc1.x, data_pc1.dcva

    def _load_save(self, i):
        if self.pre_transform is not None:
            pc0, pc1, label, feats0, feats1, dcva = self._preproc_clouds_loader(i)
        else:
            pc0, pc1, label, feats0, feats1 = self.clouds_loader(i, nameInPly=self.nameInPly, feats =True)
            if self.dcva_gt:
                dcva = self.get_DCVA_gt(self.filesPC1[i], pc1)
    ...
    def hand_craft_process(self, comp_normal=False):
        ...
        if not self.reload_preproc or not exist_file:
            for idx in range(len(self.filesPC0)):
                ...
                if self.dcva_gt:
                    gt_dcva = self.get_DCVA_gt(self.filesPC1[idx], pc1)
                    pc1 = Data(pos=pc1, y=label, x=feat1, dcva=gt_dcva)
                else:
                    pc1 = Data(pos=pc1, y=label, x=feat1, dcva=None)
                if comp_normal:
            ...
            if self.fix_cyl:
                ...
                while not pair_correct and idx < self._centres_for_sampling_fixed.shape[0]:
                    ...
                    dataPC1 = Data(pos=pair.pos_target, y=pair.y, idx=torch.arange(pair.pos_target.shape[0]).reshape(-1),
                                   gt_change=pair.gt_change, x=pair.x_target)
                    ...
                    pair_cylinders = Pair(pos=dataPC0_cyl.pos, pos_target=dataPC1_cyl.pos, y=dataPC1_cyl.y, x = dataPC0_cyl.x,
                                          ...
                                          gt_change=dataPC1_cyl.gt_change,)
                    try:
            ...
            while not pair_correct and idx<self.grid_regular_centers.shape[0]:
                ...
                try:
                    ...
                    pair_cylinders = Pair(pos=dataPC0_cyl.pos, pos_target=dataPC1_cyl.pos, y=dataPC1_cyl.y,x=dataPC0_cyl.x,
                                          ...
                                          gt_change=dataPC1_cyl.gt_change, x_target=dataPC1_cyl.x)
                    if self.DA:
    ...
    def _get_random(self):
        ...
        setattr(dataPC0, CylinderSampling.KDTREE_KEY, pair.KDTREE_KEY_PC0)
        dataPC1 = Data(pos=pair.pos_target, y=pair.y, x=pair.x_target, gt_change=pair.gt_change)
        setattr(dataPC1, CylinderSampling.KDTREE_KEY, pair.KDTREE_KEY_PC1)
        dataPC0_cyl = cylinder_sampler(dataPC0)
        dataPC1_cyl = cylinder_sampler(dataPC1)
        pair_cyl = Pair(pos=dataPC0_cyl.pos, pos_target=dataPC1_cyl.pos, y=dataPC1_cyl.y,
                        x=dataPC0_cyl.x, x_target=dataPC1_cyl.x, gt_change=dataPC1_cyl.gt_change)
        if self.DA:
    ...
    def read_from_ply(self, filename, nameInPly="params", name_feat="label_ch", n_feat=0):
        ...
        with open(filename, "rb") as f:
            ...
            # vertices[:, 4] = plydata[nameInPly].data["f{}".format(str(9))]
        return vertices

    def get_DCVA_gt(self, filename, pc, nameInPly="params"):
        filename = os.path.join(os.path.dirname(filename),os.path.basename(filename).split("feat")[0]+"_DCVA.ply")
        assert os.path.isfile(filename)
        with open(filename, "rb") as f:
            plydata = PlyData.read(f)
            num_verts = plydata[nameInPly].count
            ...
            vertices[:, 1] = plydata[nameInPly].data["y"]
            vertices[:, 2] = plydata[nameInPly].data["z"]
            vertices[:, 3] = plydata[nameInPly].data["pred"]
...
class AHNDataset_deepCluster(BaseSiameseDataset):
    def __init__(self, dataset_opt):
        ...
        self.train_dataset = AHNPairCylinder_DC(
            ...
            dcva_gt=self.dataset_opt.dcva_binary_GT,
        )
