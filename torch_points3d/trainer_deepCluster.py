...
class Trainer:
    ...
    def train_deepCluster(self):
        ...
        for epoch in range(self._checkpoint.start_epoch, self._cfg.training.epochs + 1):
            ...
            if epoch > 150 or epoch <= self._cfg.deepclustering.pre_train_kmean:  
                comp_clust = False
            else:
                comp_clust = True
            # comp_clust = True
            if comp_clust:
                ...
                centroids_cd, kmloss_cd, centroids_seg, kmloss_seg = self.run_mini_batch_kmeans(epoch,
                                                                                                ...
                                                                                                centroids_cd=None,
                                                                                                centroids_seg=None,
                                                                                                dual_task=self._cfg.deepclustering.dualTask)
                # Cluster deep features
                print('Cluster features')
                self.compute_labels(epoch, centroids_cd, centroids_seg, train_kmeans_loader,
                                    dual_task=self._cfg.deepclustering.dualTask)
                ...
                if self._cfg.deepclustering.dualTask:
                    if epoch == self._cfg.models.SiameseKPConv_DC_dualT.deepCluster.n1_epoch:
                ...
                with torch.no_grad():
                    if self._cfg.deepclustering.dualTask:
                        ...
                        elif epoch <= self._cfg.models.SiameseKPConv_DC_dualT.deepCluster.n2_epoch:
                            pass
                        else:
                            centroids_n = torch.from_numpy(centroids_cd)
                            centroids_n = torch.nn.functional.normalize(centroids_n, dim=1, p=2)
                        ...
                        centroids_n = torch.from_numpy(centroids_cd)
                        centroids_n = torch.nn.functional.normalize(centroids_n, dim=1, p=2)
                        # self._model.prototypes.weight.copy_(centroids_n)
                        self._model.prototypes_cd.prototypes_cd.weight.copy_(centroids_n)

            # if epoch == 1 or epoch % 20 == 0:
            #     # self.compute_tSNE(epoch, train_kmeans_loader)
            #     #     self._tracker.plot_save_metric_dc(plot = True, saving_path = self._cfg.output, epoch = epoch)
            #     self._tracker.save_data(self._dataset.train_data, epoch=epoch)
            # Set dataloaders
            ...
            if epoch > 150 or epoch <= self._cfg.deepclustering.pre_train_kmean:  # % self.eval_frequency == 0:
    ...
    def run_mini_batch_kmeans(self, epoch, data_loader=None, centroids_cd=None, centroids_seg=None, dual_task=False):
        """
            ...
            for i_batch, data in enumerate(tq_train_loader):
                with torch.no_grad():
                    ...
                    if dual_task:
                        ...
                        # featsegdiff_list.append(feats_seg1 - feats_seg0[self._model.nn_list_init[1, :], :])
                        # import pdb; pdb.set_trace()
                        if self._cfg.models.SiameseKPConv_DC_dualT.deepCluster.y_from_threshold and epoch==0:
                            # fdiff = torch.abs(feats_seg1 - feats_seg0[self._model.nn_list_init[1, :], :])
                        ...
                        if centroids_cd is None:
                            ...
                            if dual_task:
                                ...
                                if self._cfg.models.SiameseKPConv_DC_dualT.deepCluster.y_from_threshold and epoch==0:
                                    ...
                                    kmeans_loss_segdiff.update(loss_seg)
        ...
        if not dual_task:
            return centroids_cd, kmeans_loss_cd.avg, None, None
        else:
            ...
            return centroids_cd, kmeans_loss_cd.avg, centroids_seg, kmeans_loss_seg.avg

    def compute_labels(self, epoch, centroids_cd, centroids_seg=None, data_loader=None, dual_task=False):
        """
        ...
        if epoch > 150:  # % self.eval_frequency == 0: #>= pour qu'au dernier kmean soit calculé les plab du val
            ...
            for i, data in enumerate(tq_loader):
                with torch.no_grad():
                    ...
                    if self.index_segdiff is not None:
                        # feats_segdiff = np.abs(feats_seg1 - feats_seg0[self._model.nn_list_init[1, :].cpu().numpy(), :])
