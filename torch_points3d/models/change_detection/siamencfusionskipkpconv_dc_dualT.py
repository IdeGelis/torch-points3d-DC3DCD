...
class BaseFactoryPSI:
    def __init__(self, module_name_down_cd, module_name_down_seg, module_name_up_cd, module_name_up_seg, modules_lib):
        self.module_name_down_cd = module_name_down_cd
        self.module_name_down_seg = module_name_down_seg
    ...
    def get_module(self, flow):
        ...
        if flow.upper() == "DOWN_CONV_SEG":
            return getattr(self.modules_lib, self.module_name_down_seg, None)
        if flow.upper() == "DOWN_CONV_CD":
            return getattr(self.modules_lib, self.module_name_down_cd, None)


####################DEEPCLUSTER SIAMESE KP CONV Dual Task############################
class SiamEncFusionSkipKPConv_DC_dualT(UnwrappedUnetBasedModel):
    ...
    def print_nb_param(self, log):
        ...
        log.info('downconv cd: ' + str(sum(p.numel() for p in self.down_modules_cd.parameters() if p.requires_grad)))
        log.info('downconv seg: ' + str(sum(p.numel() for p in self.down_modules_seg.parameters() if p.requires_grad)))
    ...
    def _init_from_compact_format(self, opt, model_type, dataset, modules_lib):
        ...
        in lists
        """
        self.down_modules_seg = nn.ModuleList()
        self.down_modules_cd = nn.ModuleList()
        self.inner_modules = nn.ModuleList()
        self.up_modules_seg = nn.ModuleList()
        self.up_modules_cd = nn.ModuleList()

        self.save_sampling_id = opt.down_conv_cd.get('save_sampling_id')

        # Factory for creating up and down modules
        factory_module_cls = self._get_factory(model_type, modules_lib)
        down_conv_cd_cls_name = opt.down_conv_cd.module_name
        down_conv_seg_cls_name = opt.down_conv_seg.module_name
        ...
        self._factory_module = factory_module_cls(
            down_conv_cd_cls_name,down_conv_seg_cls_name, up_conv_cd_cls_name, up_conv_seg_cls_name, modules_lib
        ...
        # Down modules
        for i in range(len(opt.down_conv_seg.down_conv_nn)):
            args = self._fetch_arguments(opt.down_conv_seg, i, "DOWN_CONV_SEG")
            conv_cls = self._get_from_kwargs(args, "conv_cls")
            down_modules = conv_cls(**args)
            self._save_sampling_and_search(down_modules)
            self.down_modules_seg.append(down_modules)

        for i in range(len(opt.down_conv_cd.down_conv_nn)):
            args = self._fetch_arguments(opt.down_conv_cd, i, "DOWN_CONV_CD")
            conv_cls = self._get_from_kwargs(args, "conv_cls")
            down_modules = conv_cls(**args)
            self._save_sampling_and_search(down_modules)
            self.down_modules_cd.append(down_modules)
    ...
    def forward(self, compute_loss=True, epoch=0, *args, **kwargs) -> Any:
        ...
        data0 = self.input0.clone()
        data1 = self.input1.clone()

        data0 = self.down_modules_seg[0](data0, precomputed=self.pre_computed)
        data1 = self.down_modules_seg[0](data1, precomputed=self.pre_computed_target)
        data_diff = self.down_modules_cd[0](self.input1.clone(), precomputed=self.pre_computed_target)
        stack_down_input0.append(data0)
        stack_down_input1.append(data1)
        self.nn_list_init = knn(data0.pos, data1.pos, 1, data0.batch, data1.batch)
        diff = data1.clone()
        diff.x = data1.x - data0.x[self.nn_list_init[1, :], :]
        data_diff.x = torch.cat((data1.x, diff.x), axis=1)
        stack_down_CD.append(data_diff)


        for i in range(1, len(self.down_modules_seg) - 1):
            data0 = self.down_modules_seg[i](data0, precomputed=self.pre_computed)
            data1 = self.down_modules_seg[i](data1, precomputed=self.pre_computed_target)
            data_diff = self.down_modules_cd[i](data_diff, precomputed=self.pre_computed_target)
            nn_list = knn(data0.pos, data_diff.pos, 1, data0.batch, data_diff.batch)
            diff = data_diff.clone()
            diff.x = data_diff.x - data0.x[nn_list[1, :], :]
            data_diff.x = torch.cat((data_diff.x, diff.x), axis=1)
            stack_down_CD.append(data_diff)
            stack_down_input0.append(data0)
            stack_down_input1.append(data1)

        # 1024 : last layer
        data0 = self.down_modules_seg[-1](data0, precomputed=self.pre_computed)
        data1 = self.down_modules_seg[-1](data1, precomputed=self.pre_computed_target)
        data_diff = self.down_modules_cd[-1](data_diff, precomputed=self.pre_computed_target)

        nn_list = knn(data0.pos, data_diff.pos, 1, data0.batch, data_diff.batch)
        diff = data_diff.clone()
        diff.x = diff.x - data0.x[nn_list[1, :], :]
        data_diff.x = torch.cat((data_diff.x, diff.x), axis=1)
        innermost = False
        if not isinstance(self.inner_modules[0], Identity):
            stack_down.append(data1)
            data_diff = self.inner_modules[0](data_diff)
            innermost = True

        for i in range(len(self.up_modules_cd)):
            if i == 0 and innermost:
                data_diff = self.up_modules_cd[i]((data_diff, stack_down_CD.pop()))
            else:
                data_diff = self.up_modules_cd[i]((data_diff, stack_down_CD.pop()), precomputed=self.upsample_target)
        ...
        self.last_features_cd = self.FC_layer_cd(data_diff.x)
    ...
    def compute_loss(self, epoch=0):
        ...
        if epoch <= self.option.deepCluster.nSeg_epoch:
    ...
    def backward(self):
        ...
        if self.ep_cd:
            layer = ["prototypes_cd", "prototypes_seg", "down_modules_seg", "up_modules_seg","FC_layer_seg" ]
        elif self.ep_seg:
            layer = ["prototypes_cd", "prototypes_seg", "down_modules_cd", "up_modules_cd", "FC_layer_cd"]
        else:
            layer = ["prototypes_cd", "prototypes_seg" ]
    ...
    def get_max_margin_contrastive_loss_seg(self):
        ...
        if self.option.deepCluster.y_from_threshold:
            ...
            y[torch.argmax(self.output_seg_d0, dim=1)[self.nn_list_init[1, :]] == torch.argmax(self.output_seg_d1, dim=1)] = 1
        ...
        loss_cont = beta * (0.5 * y * dFseg ** 2 + (1 - y) * torch.max(self.option.deepCluster.alpha - dFseg, zero)** 2)
    ...
    def set_pretrained_weights(self):
        super(SiamEncFusionSkipKPConv_DC_dualT, self).set_pretrained_weights()
        # Concerning encoders only now
        path_pretrained = getattr(self.opt, "path_pretrained_seg", None)
        ...
        if path_pretrained is not None:
            if not os.path.exists(path_pretrained):
                ...
                self.load_state_dict_seg(m, strict=False)
    ...
    def modif_name_dict(self, weights, model_state):
        ...
        for k, v in weights.items():
            ...
            elif "down_modules" in k:
                knew = k.replace("down_modules", "down_modules_seg")
                if knew in model_state and v.size() == model_state[knew].size():
                    filtered_weights[knew] = v
        return filtered_weights

    def load_state_dict_seg(self, weights, strict=False):
        model_state = self.state_dict()
        filtered_weights = self.modif_name_dict(weights, model_state)
        filtered_weights = {k: v for k, v in filtered_weights.items() if
                            'seg' in k and (k in model_state and v.size() == model_state[k].size())}
        log.info("Loading weights:" + ", ".join(filtered_weights.keys()))
        self.load_state_dict(filtered_weights, strict=strict)
        for p in self.down_modules_seg.parameters():
            if p.requires_grad:
                p.requires_grad = False
        for p in self.up_modules_seg.parameters():
            if p.requires_grad:
                p.requires_grad = False
